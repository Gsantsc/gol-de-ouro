...
export const submitPrediction = async ({
  ...
  const { error } = await supabase.rpc("submit_prediction", rpcPayload);
  if (!error) return;

  if (shouldFallbackToPredictionUpsert(error)) {
    debugWarn("[MOBILE PREDICTION] submit_prediction RPC unavailable. Trying direct upsert fallback.");
    const { error: upsertError } = await supabase
      .from("predictions")
      .upsert(directPayload, { onConflict: "user_id,match_id" });

    if (!upsertError) return;
    debugWarn("[MOBILE PREDICTION] direct upsert failed", readSupabaseErrorMessage(upsertError));
    throw new Error(predictionSubmitMessage(upsertError));
  }

  debugWarn("[MOBILE PREDICTION] submit failed", readSupabaseErrorMessage(error));
