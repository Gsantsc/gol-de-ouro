...
const requestApiFootball = async <T>(
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.name === "AbortError") {
        throw new Error("API-Football: Tempo excedido. Servidor não respondeu em 30 segundos.");
      }
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      throw new Error("API-Football: Chave de API inválida ou expirada. Verifique API_FOOTBALL_KEY.");
    }
