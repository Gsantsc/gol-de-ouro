...
const values = dataset.matches.map((match) => `(
  ...
  ${sqlJson({ city: match.city, country: match.country, group: match.group, match_number: match.matchNumber, source: "static-world-cup-2026-dataset", stage: match.stage })}
)`).join(",\n");
...
), dataset(match_number, home_team, away_team, start_time, prediction_open_at, prediction_close_at, status, championship, stadium, round, provider_name, provider_external_id, live_score, stats) as (
  values
${values}
)
